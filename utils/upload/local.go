...
func (l Local) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	// 尝试创建此路径
	mkdirErr := os.MkdirAll(global.G_CONFIG.Local.Path, os.ModePerm)
	if mkdirErr != nil {
		global.G_LOG.Error("function os.MkdirAll() Filed", zap.Any("err", mkdirErr.Error()))
		return "", "", errors.New("function os.MkdirAll() Filed, err:" + mkdirErr.Error())
	}
	// 拼接路径和文件名
	filePath := path.Join(global.G_CONFIG.Local.Path, filename)
...
func (l Local) DeleteFile(key string) error {
	p := path.Join(global.G_CONFIG.Local.Path, key)
	if strings.Contains(p, global.G_CONFIG.Local.Path) {
